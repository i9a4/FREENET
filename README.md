...
> [!WARNING]
> ВНИМАНИЕ! В СЛУЧАЕ ПРИНЯТИЯ В СИЛУ СТАТЬИ 13.53 КОАП РФ, НЕ ИСПОЛЬЗУЙТЕ СРЕДСТВА ОБХОДА, КРОМЕ КАК VPN И ПОДОБНЫХ СРЕДСТВ, КОТОРЫЕ ПОЛНОСТЬЮ ОБВОРАЧИВАЮТ ТРАФИК. ВЫ МОЖЕТЕ БЫТЬ ПРИВЛЕЧЕНЫ К АДМИНИСТРАТИВНОЙ ОТВЕТСТВЕННОСТИ, А ФАКТ ОБХОДА ФИЛЬТРОВ РКН БУДЕТ ТОЛЬКО ПОДКРЕПЛЯТЬ ДОВОДЫ ПРОКУРОРА О НАМЕРЕННОМ ПОИСКЕ ЧЕГО-ЛИБО. НЕ ЗАБЫВАЙТЕ, ЧТО ПОСЛЕ ПРИНЯТИЯ ПАКЕТА ЯРОВОЙ ВЕСЬ ТРАФИК, КОТОРЫЙ ПРОХОДИТ ЧЕРЕЗ ПРОВАЙДЕРА, ИДЕТ НЕ ТОЛЬКО ЧЕРЕЗ НЕГО, НО И ЧЕРЕЗ УСТРОЙСТВО COMP, ДОСТУП К КОТОРОМУ ИМЕЕТ ФСБ.

> [!NOTE]
> Решение может быть неактуально для некоторых пользователей. Разработчик GoodByeDPI на данный момент не выпускает обновлений. Используйте [WARP+](https://one.one.one.one/). При необходимости, можно совместить использование программы, но без включения GoodBYEDPI.
